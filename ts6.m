clc;
clearvars -except X1 Y1x Y1y X2 Y2x Y2y X3 Y3x Y3y X4 Y4x Y4y X5 Y5x Y5y;
%%%
coord = readmatrix("gait_data_6.xlsx");
time = coord(:,1);
tfit = time;
tt = time;
%%%
...
%%% GRFs
f = readmatrix("grf_data_6.xlsx");
tf = f(:,1);
gt = 1:15:3046;
GRFx = f(gt,2);
GRFy = f(gt,3);
mark = 6:133; % full gait cycle
%mark = 6:89; % non zero GRF
%mark = 6:24; % first DS
%mark = 70:89; % second DS
%%%
...
Ymx = [Y1x;Y2x;Y3x;Y4x;Y5x;Y6x];
Ymy = [Y1y;Y2y;Y3y;Y4y;Y5y;Y6y];
%%%
betax = ridge(Ymx,Xm,1,0); %ridge
betay = ridge(Ymy,Xm,1,0); %ridge
% lasso
[Bx,FitInfox] = lasso(Xm,Ymx,'Alpha',0.75,'CV',10);
idxLambda1SEx = FitInfox.Index1SE;
coefx = Bx(:,idxLambda1SEx);
coef0x = FitInfox.Intercept(idxLambda1SEx);
[By,FitInfoy] = lasso(Xm,Ymy,'Alpha',0.75,'CV',10);
idxLambda1SEy = FitInfoy.Index1SE;
coefy = By(:,idxLambda1SEy);
coef0y = FitInfoy.Intercept(idxLambda1SEy);