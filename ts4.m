clc;
clearvars -except X1 Y1x Y1y X2 Y2x Y2y X3 Y3x Y3y;
%%%
coord = readmatrix("gait_data_4.xlsx");
time = coord(:,1);
tfit = time;
tt = time;
%%%
...
%%% GRFs
f = readmatrix("grf_data_4.xlsx");
tf = f(:,1);
gt = 1:15:2671;
GRFx = f(gt,2);
GRFy = f(gt,3);
mark = 19:132; % full gait cycle
%mark = 63:132; % non zero GRF
%mark = 63:76; % first DS
%mark = 118:132; % second DS
%%%
